...
export default function handleMouseDown(this: App, e: MouseEvent) {
	...
	if (e.button === 0) {
		...
		if (targetClasses.contains("textEditorChar")) {
			...
			let editorFontSize = document.getElementById("text-editor-font-size");
			if (editorFontSize != null) {
				let val = parseInt((editorFontSize as HTMLInputElement).value);
				if (val >= 6 && val <= 60) {
					nextState.settings.textEditorFontSize = parseInt((editorFontSize as HTMLInputElement).value);
					(document.querySelector(":root") as HTMLElement).style.setProperty("--text-editor-font-size", nextState.settings.textEditorFontSize + "pt");
				}
			}
			let outputFontSize = document.getElementById("output-font-size");
			if (outputFontSize != null) {
				let val = parseInt((outputFontSize as HTMLInputElement).value);
				if (val >= 6 && val <= 60) {
					nextState.settings.outputFontSize = parseInt((outputFontSize as HTMLInputElement).value);
					(document.querySelector(":root") as HTMLElement).style.setProperty("--output-view-font-size", nextState.settings.outputFontSize + "pt");
				}
			}
			nextState.settings.visible = false;
		}
